 ...
 * leader across every measure, which is the thing a comparison is actually
 * for: the leader is rarely the leader on all of them.
 *
...
export interface PodiumEntry {
  ...
  value: string;
  /** "total pay a year · L3 · Madrid" for the leader; "−€17.9k behind" after. */
  detail?: string;
}
    ...
    <section className="mb-7 rounded-[20px] bg-eq-accent px-6 py-7 text-eq-accent-foreground shadow-[0_10px_34px_rgb(36_56_46_/_18%)] sm:px-9 sm:py-8">
      ...
      {leader === undefined ? (
        ...
        <ol className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-stretch">
          <li className="min-w-0 rounded-2xl bg-eq-accent-foreground/[0.12] px-5 py-5 sm:flex-[1.6]">
            <div className="flex items-center gap-2.5">
              <Place position={1} />
              <p className="min-w-0 truncate text-[clamp(1.25rem,2.4vw,1.625rem)] font-semibold tracking-[-0.024em]">
                {leader.name}
              </p>
            </div>
            <p className="mt-3.5 text-[clamp(2rem,4.2vw,3rem)] font-semibold leading-[0.95] tracking-[-0.033em] tabular">
              {leader.value}
            </p>
            {leader.detail && <p className="mt-2 text-xs opacity-60">{leader.detail}</p>}
          </li>

          {rest.map((entry, index) => (
            <li
              key={entry.slug}
              className="min-w-0 rounded-2xl bg-eq-accent-foreground/[0.07] px-4 py-4 sm:flex-1"
            >
              <div className="flex items-center gap-2">
                <Place position={index + 2} />
                <p className="min-w-0 truncate text-[17px] font-semibold tracking-[-0.016em]">
                  {entry.name}
                </p>
              </div>
              <p className="mt-2.5 text-[26px] font-semibold leading-none tracking-[-0.024em] tabular">
                {entry.value}
              </p>
              {entry.detail && (
                <p className="mt-1.5 text-[11.5px] opacity-55">{entry.detail}</p>
              )}
            </li>
          ))}
        </ol>
    ...
    <span
      ...
      className={`grid size-[19px] shrink-0 place-items-center rounded-full text-[10.5px] font-bold ${
        position === 1
          ...
          : "bg-eq-accent-foreground/25 text-eq-accent-foreground"
      }`}
    >
      {position}
    </span>
