...
import { CaretDown, CaretUp, Check } from "@/components/eq/icon"
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-2 rounded-lg py-2 pr-8 pl-2.5 text-xs font-medium outline-hidden select-none data-highlighted:bg-muted data-highlighted:text-foreground data-selected:bg-primary/8 data-selected:text-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className
      )}
      {...props}
    >
      ...
      <SelectPrimitive.ItemIndicator
        render={
          <span className="pointer-events-none absolute right-2.5 flex size-3.5 items-center justify-center text-primary" />
        }
      >
        <Check className="pointer-events-none size-3.5" weight="bold" />
      </SelectPrimitive.ItemIndicator>
    </SelectPrimitive.Item>
  )
}
