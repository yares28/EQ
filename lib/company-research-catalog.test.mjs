...
function catalogPoint(overrides = {}) {
  return {
    companySlug: "newco",
    level: "intern",
    location: "Madrid",
    ...
    extrasEur: null,
    confidence: "Medium",
    confidenceNote: "Two independent public salary pages agreed within 10%.",
    notes: "levels.fyi + Glassdoor",
    sources: [
      {
        label: "NewCo intern salaries",
        url: "https://www.levels.fyi/companies/newco/salaries",
        publisher: "Levels.fyi",
        checkedAt: "2026-09-01",
      },
    ],
    researchedAt: Date.UTC(2026, 8, 1),
    ...overrides,
  };
}

test("a pasted company outside the compiled catalog can carry a researched figure", () => {
  // Before this, the tracked-only branch passed no salary evidence at all, so
  // this company could never show a number however much research existed.
  const catalog = buildCompanyResearchCatalog({
    baseCompanies: [],
    trackedCompanies: [tracked("newco")],
    postedRanges: [],
    catalogPoints: [catalogPoint()],
  });
  const company = catalog.find((entry) => entry.slug === "newco");
  assert.equal(company.salaryPoints.length, 1);
  assert.equal(company.salaryPoints[0].level, "intern");
  assert.equal(company.salaryPoints[0].totalCompEur, 24_000);
  assert.equal(company.sources.length, 1);
  assert.equal(company.sources[0].publisher, "Levels.fyi");
});

test("an employer-posted range outranks a researched figure at the same level", () => {
  const catalog = buildCompanyResearchCatalog({
    baseCompanies: [],
    trackedCompanies: [tracked("newco", { researchStatus: "monitoring" })],
    postedRanges: [range()],
    catalogPoints: [catalogPoint({ level: "junior", totalCompEur: 40_000 })],
  });
  const company = catalog.find((entry) => entry.slug === "newco");
  const junior = company.salaryPoints.filter((point) => point.level === "junior");
  assert.equal(junior.length, 1);
  assert.match(junior[0].id, /^posted:/);
});

test("a researched figure fills only levels the postings leave empty", () => {
  const catalog = buildCompanyResearchCatalog({
    baseCompanies: [],
    trackedCompanies: [tracked("newco", { researchStatus: "monitoring" })],
    postedRanges: [range()],
    catalogPoints: [catalogPoint({ level: "intern" })],
  });
  const company = catalog.find((entry) => entry.slug === "newco");
  const levels = company.salaryPoints.map((point) => point.level).sort();
  assert.deepEqual(levels, ["intern", "junior"]);
});

test("a researched senior figure never appears at another level", () => {
  const catalog = buildCompanyResearchCatalog({
    baseCompanies: [],
    trackedCompanies: [tracked("newco")],
    postedRanges: [],
    catalogPoints: [catalogPoint({ level: "senior", totalCompEur: 90_000 })],
  });
  const company = catalog.find((entry) => entry.slug === "newco");
  assert.deepEqual(company.salaryPoints.map((point) => point.level), ["senior"]);
});

test("a researched figure does not displace the compiled catalog at other levels", () => {
  const catalog = buildCompanyResearchCatalog({
    baseCompanies: [baseCompany("known")],
    trackedCompanies: [],
    postedRanges: [],
    catalogPoints: [catalogPoint({ companySlug: "known", level: "intern" })],
  });
  const company = catalog.find((entry) => entry.slug === "known");
  const levels = company.salaryPoints.map((point) => point.level).sort();
  assert.deepEqual(levels, ["intern", "junior"]);
});
