...
test("reads a slash pay period whether the employer spaces it or writes it in Spanish", () => {
  // BNP Paribas' own BECA Data Engineer posting states the stipend as
  // "brutos/mes", with no space around the slash.
  const spanish = parseCompanyPostedSalary({
    title: "Software Engineer Intern",
    locations: ["Madrid, Spain"],
    salaryText: "Remuneración: 880€ brutos/mes.",
  });

  assert.equal(spanish.accepted, true);
  assert.equal(spanish.period, "month");
  assert.equal(spanish.canonicalLevel, "intern");
  assert.equal(spanish.minimumAmount, 880);
  assert.ok(spanish.qualityFlags.includes("explicit_period"));

  // Procter & Gamble writes the same unit with spaces around the slash.
  const spaced = parseCompanyPostedSalary({
    title: "Software Engineer Internship",
    locations: ["Madrid, Spain"],
    salaryText: "€1.200,00 / month",
  });

  assert.equal(spaced.accepted, true);
  assert.equal(spaced.period, "month");
  assert.equal(spaced.minimumAmount, 1_200);
  assert.ok(spaced.qualityFlags.includes("explicit_period"));
  // Before the spaced slash was recognised this text fell through to the
  // annual inference, which read a €1,200 monthly stipend as a yearly salary.
  assert.ok(!spaced.qualityFlags.includes("inferred_annual_period"));
});

test("a slash before a place name is not a pay period", () => {
  const result = parseCompanyPostedSalary({
    title: "Senior Software Engineer",
    locations: ["Barcelona, Spain"],
    salaryText: "Barcelona/Montpellier · €70.000 – €80.000",
  });

  assert.notEqual(result.period, "month");
  assert.equal(result.period, "year");
  assert.ok(result.qualityFlags.includes("inferred_annual_period"));
});

test("the harvested data-engineering stipends parse their period but stay out of the catalog", () => {
  // Both figures now read correctly; the catalog still excludes them because
  // `canonicalOccupation` scopes it to software-engineering IC titles. That
  // scope is a product decision, so this test pins the current behaviour
  // rather than asserting it is the right one.
  for (const posting of [
    { title: "BECA Data Engineer", salaryText: "Remuneración: 880€ brutos/mes.", amount: 880 },
    { title: "Data Engineer Internship", salaryText: "€1.200,00 / month", amount: 1_200 },
  ]) {
    const result = parseCompanyPostedSalary({
      title: posting.title,
      locations: ["Madrid, Spain"],
    ...
    assert.equal(result.minimumAmount, posting.amount);
    assert.equal(result.accepted, false);
    assert.ok(result.rejectionReasons.includes("not_software_engineering_ic"));
  }
});
