...
/**
 * Every string below is one an employer's own feed actually writes, read from
 * Airbus's and P&G's Workday location facets with the country facet pinned to
 * Spain. Neither feed states the country anywhere in the posting record, so a
 * string missing here is a role that can never reach the Spain archive.
 */
test("Spain detection reads the site names employers write instead of the country", () => {
  for (const location of [
    // Airbus (ag.wd3.myworkdayjobs.com/Airbus), by descending role count.
    ["Getafe Area"], ["Albacete"], ["Sevilla Area"], ["Madrid Area"],
    ["Illescas"], ["Getafe"], ["Cadiz Area"], ["Barcelona"], ["Getafe (Ensia)"],
    // P&G (pg.wd5.myworkdayjobs.com/1000).
    ["MADRID GENERAL OFFICE"], ["MEQUINENZA PLANT"], ["JIJONA PLANT"],
    ["Mequinenza"], ["Madrid"], ["Jijona  Alicante"], ["Castellon"],
    ["Palma de Mallorca"],
    // A multi-site role keeps its Spanish leg.
    ["Getafe", "Sevilla Area"],
  ]) {
    assert.equal(isSpainLocation(location), true, `${location} is Spain`);
  }
...
test("a Spanish city name qualified as somewhere else stays out", () => {
  for (const location of [
    ["Valencia, CA"], ["Valencia, California"], ["Cadiz, OH"], ["Cadiz, Ohio"],
    ["Madrid, NM"], ["Madrid, Iowa"], ["Barcelona, Venezuela"],
    ["Malaga, NJ"], ["Sevilla, Colombia"], ["Illescas, Peru"],
    ["Valencia, Philippines"], ["Madrid, USA"],
  ]) {
    assert.equal(isSpainLocation(location), false, `${location} is not Spain`);
  }
  // The veto is a qualifier, not a substring: a Spanish region that merely
  // starts with a state code must not knock its own city out.
  for (const location of [
    ["Barcelona, Cataluña"], ["Valencia, Castellón"], ["Madrid, Comunidad de Madrid"],
    ["Cádiz, Andalucía"], ["Málaga, Andalucía, Spain"], ["Madrid, in Spain"],
  ]) {
    assert.equal(isSpainLocation(location), true, `${location} is Spain`);
  }
});

test("the salary parser and the scope check read a site name the same way", () => {
  const result = parseCompanyPostedSalary({
    title: "Senior Software Engineer",
    locations: ["Getafe Area"],
    salaryText: "€45,000 - €55,000 per year",
  });

  assert.equal(isSpainLocation(["Getafe Area"]), true);
  assert.equal(result.accepted, true);
  assert.equal(result.countryCode, "ES");
  assert.equal(result.cityKey, "getafe");
  assert.equal(result.locationLabel, "Getafe");
  assert.equal(postedSalaryLocationLabel("getafe", "Getafe Area"), "Getafe");
});

test("a site the scope check rejects yields no Spanish pay either", () => {
  const result = parseCompanyPostedSalary({
    title: "Senior Software Engineer",
    locations: ["Cadiz, OH"],
    salaryText: "€45,000 - €55,000 per year",
  ...
  assert.equal(isSpainLocation(["Cadiz, OH"]), false);
  assert.equal(result.accepted, false);
  assert.equal(result.countryCode, "XX");
  assert.ok(result.rejectionReasons.includes("outside_spain_scope"));
});
