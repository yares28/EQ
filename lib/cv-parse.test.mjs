...
import { parseCvText, repairAccents } from "./cv-parse.ts";

/**
 * The text pdfjs actually produces from public/YahyaFaresENG.pdf, taken from
 * the live import rather than typed by hand — long bullets wrap across lines,
 * columns are separated by runs of spaces, and LaTeX accents arrive split.
 * The hand-written fixture below used single-line bullets and hid all of that.
 */
const REAL_EXTRACTION = `Yahya Fares
...
Amazon   Madrid, Spain
\u2022   Designed and shipped an internal automation tool for shift distribution and workforce allocation, eliminating hours
of manual coordination per week and materially improving scheduling efficiency.
\u2022   Built an internal workforce forecasting application used by HR the labor planning team to predict daily/weekly
headcount from site rosters by modeling planned and unplanned absences, improving staffing decisions. Used
internal AWS full stack integrated components (EC2, DynamoDB, Lambda, AWS Amplify...).
\u2022   Led teams of 40+ associates across multiple operational areas, consistently hitting productivity, safety, and quality
targets in a high-volume, KPI-driven environment.
\u2022   Built lightweight reporting workflows to surface real-time performance signals, enabling faster escalation and
corrective action across shifts.
Software Developer Intern   Jul 2021 – Sep 2023
FacStructure   Casablanca, Morocco
\u2022   Selected to return for three consecutive summer internships (2021, 2022, 2023) based on consistent delivery and
ownership of high-impact projects.
\u2022   Architected a relational database schema on AWS and led end-to-end migration from physical archives to SQL
Server, improving data retrieval performance and ensuring integrity across tens of thousands of records.`;

test("real wrapped bullets stay one bullet each", () => {
  const cv = parseCvText(REAL_EXTRACTION);
  const experience = cv.sections.find((section) => section.heading === "Experience");
  assert.equal(experience.entries.length, 2, "two jobs, not one per wrapped line");

  const amazon = experience.entries[0];
  assert.equal(amazon.title, "Amazon Operations Intern");
  assert.equal(amazon.subtitle, "Amazon");
  assert.equal(amazon.bullets.length, 4);
  // The first bullet's continuation line must be joined onto it, not orphaned.
  assert.match(amazon.bullets[0].text, /eliminating hours of manual coordination per week/);
  assert.match(amazon.bullets[1].text, /EC2, DynamoDB, Lambda, AWS Amplify/);

  const fac = experience.entries[1];
  assert.equal(fac.title, "Software Developer Intern");
  assert.equal(fac.subtitle, "FacStructure");
  assert.equal(fac.bullets.length, 2);
});

test("a project keeps its stack in the title and its link in the meta", () => {
  // Real extraction: "Trakzi   |   Next.js, ...   www.trakzi.com". Splitting at
  // the first run of spaces left the title as bare "Trakzi".
  const cv = parseCvText(`Projects
Trakzi   |   Next.js, React, Typescript, PostreSQL, Redis, Gemini   www.trakzi.com
\u2022   Built an all-in-one personal finance app.`);
  const project = cv.sections[0].entries[0];
  assert.equal(project.title, "Trakzi | Next.js, React, Typescript, PostreSQL, Redis, Gemini");
  assert.equal(project.meta, "www.trakzi.com");
});

test("a wrapped skills line stays one line, not a phantom entry", () => {
...
Tools : Git, Postman, AWS, Docker, Kubernetes`);
  ...
  assert.equal(section.entries.length, 0, "no entry should be invented from a wrapped skills line");
  assert.equal(section.looseLines.length, 2);
  assert.match(section.looseLines[0], /Next\.js, HTML, CSS, TailwindCSS/);
});

test("split LaTeX accents are repaired", () => {
  assert.equal(repairAccents("Polit\u00b4 ecnica"), "Politécnica");
  assert.equal(repairAccents("Espa~ na"), "Españ a".replace(" ", ""));
  const cv = parseCvText(REAL_EXTRACTION);
  assert.match(cv.education[0], /Politécnica/);
});
