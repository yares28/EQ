...
test("splitSearchedLevels separates a searched dead end from work never started", () => {
  const missing = ["intern", "junior", "mid"];
  const split = splitSearchedLevels(missing, [
    { level: "intern" },
    // A check at a level that is not missing must not remove anything.
    { level: "senior" },
  ]);
  assert.deepEqual(split.checkedEmptyLevels, ["intern"]);
  assert.deepEqual(split.unsearchedLevels, ["junior", "mid"]);
});

test("splitSearchedLevels reports everything unsearched when nothing was checked", () => {
  const split = splitSearchedLevels(["intern", "junior", "mid"], []);
  assert.deepEqual(split.checkedEmptyLevels, []);
  assert.deepEqual(split.unsearchedLevels, ["intern", "junior", "mid"]);
});

test("splitSearchedLevels reports nothing left to search once every level was checked", () => {
  const split = splitSearchedLevels(["intern", "junior"], [
    { level: "junior" },
    { level: "intern" },
  ]);
  assert.deepEqual(split.checkedEmptyLevels, ["intern", "junior"]);
  assert.deepEqual(split.unsearchedLevels, []);
});
