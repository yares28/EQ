...
const EUROPEAN_LOCATION = /(?:\b(spain|españa|madrid|barcelona|málaga|malaga|bilbao|sevilla|seville|zaragoza|alicante|europe|european union|eu|emea|united kingdom|uk|london|ireland|dublin|germany|berlin|munich|france|paris|netherlands|amsterdam|switzerland|zurich|sweden|stockholm|portugal|lisbon|italy|milan|poland|warsaw|austria|vienna|denmark|copenhagen|finland|helsinki|norway|oslo|czechia|prague|belgium|brussels|luxembourg|romania|estonia|lithuania|latvia|greece)\b|(?:^|[\s,/(])valència(?:$|[\s,)/])|\bcomunitat valenciana\b|\bcomunidad valenciana\b)/i;
...
export function isRelevantToSpainSoftware(title: string, locations: string[]): boolean {
  ...
  const valenciaSpain = /(?:\bvalencia\b|(?:^|[\s,/(])valència(?:$|[\s,)/]))/i.test(location) &&
    !/\bvalencia\s*,\s*(?:ca|california)\b/i.test(location);
  const europeanLocation = EUROPEAN_LOCATION.test(location) || valenciaSpain;
